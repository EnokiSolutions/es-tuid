...
{
  [Wire]
  public sealed class TuidGenerator : ITuidGenerator
  {
    ...
    public TuidGenerator(ulong nodeId = 256, Func<ulong> clock = null, Func<ulong> random = null)
    {
      _clock = clock;
      _random = random;
    ...
    public string Generate()
    {
      ...
      var e = ((_nodeId & 0x3f) << 2) | (rand1 & 0x3ff); // bits 12..00
      var f = rand1 >> 32; // bits 64..32
      return $"{a:x08}-{b:x04}-{c:x04}-{d:x04}-{e:x04}{f:x08}";
    }
  }

  public interface ITuidGenerator
  {
    string Generate();
  }
}